...
Public Function Initialize(arg_oCropsConfig As CropsConfig, arg_sName As String, arg_cCompostReference As Range, arg_oCropRotationPart As CropRotationPart)
    Set oCropRotationPart = arg_oCropRotationPart
    Set cCompostReference = arg_cCompostReference
    Set oCropsConfig = arg_oCropsConfig
    
    ' get data
    Set dDataValues = arg_oCropsConfig.FieldWorkConfigValues(arg_sName)
    
    ' calculate data
    dDataValues("Deckungsbeitrag") = -1 * dDataValues("Ertrag bzw. Aufwand") * dDataValues("Verkaufs- bzw. Einkaufspreis") - dDataValues("Summe variable Kosten")
...
Public Function CompostInitialize()
    If InStr(1, dData("Frucht bzw. Feldarbeit"), "Kompost") = 1 Then
        dDataValues("Ertrag bzw. Aufwand") = oCropsConfig.CompostConfig.TotalWeight / oCropRotationPart.CropRotation.AccDataValue("Fläche", "Kompost", False)
        dDataValues("Stickstoff") = oCropsConfig.CompostConfig.InterpolatedDataValue("Stickstoff")
        dDataValues("Phosphor") = oCropsConfig.CompostConfig.InterpolatedDataValue("Phosphor")
        dDataValues("Kalium") = oCropsConfig.CompostConfig.InterpolatedDataValue("Kalium")
        dDataValues("Schwefel") = oCropsConfig.CompostConfig.InterpolatedDataValue("Schwefel")
        dDataValues("Calcium") = oCropsConfig.CompostConfig.InterpolatedDataValue("Calcium")
        dDataValues("Magnesium") = oCropsConfig.CompostConfig.InterpolatedDataValue("Magnesium")
        dDataValues("Bor") = oCropsConfig.CompostConfig.InterpolatedDataValue("Bor")
        dDataValues("Kupfer") = oCropsConfig.CompostConfig.InterpolatedDataValue("Kupfer")
        dDataValues("Mangan") = oCropsConfig.CompostConfig.InterpolatedDataValue("Mangan")
        dDataValues("Zink") = oCropsConfig.CompostConfig.InterpolatedDataValue("Zink")
        LateLateInitialize2
    End If
End Function
...
Public Function LateLateInitialize2()
